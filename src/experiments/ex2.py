...
def interpret_results(counts, state):
    # Interpretation of measurement results
    print("\nInterpretation of Measurement Results:")
    dominant_state = max(counts, key=counts.get)  # Find the most common measurement
    print(f"Dominant measurement state: {dominant_state} with frequency {counts[dominant_state]}")
    print("This suggests the system's radiation is highly deterministic, indicating minimal entropy or maximal coherence in the emitted radiation qubits.")

    # Interpretation of the final statevector
    print("\nInterpretation of Final Statevector:")
    max_amplitude_index = max(range(len(state.data)), key=lambda i: abs(state.data[i]))
    max_amplitude_state = f"{max_amplitude_index:0{state.num_qubits}b}"
    print(f"State with the highest amplitude: |{max_amplitude_state}> with amplitude {state.data[max_amplitude_index]}")
    print("The statevector indicates that the black hole-radiation system remains coherent despite entanglement. This coherence could hint at unitarity preservation in the simulated black hole evaporation process.")
...
interpret_results(counts, state)
