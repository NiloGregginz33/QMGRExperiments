...
class ScaledEmergentSpacetime:
    ...
    def run_entropy_area_experiment(self):
        ...
        for phi_val in self.timesteps:
            ...
            task = self.device.run(rm_circ, inputs={"phi": phi_val}, shots=2048)
            result = task.result()
...
class AdSGeometryAnalyzer:
    ...
    def run_experiment(self):
        ...
        for phi_val in self.timesteps:
            ...
            rm_circ, mapping = remap_circuit_to_contiguous_qubits(circ)
            task = self.device.run(rm_circ, inputs={"phi": phi_val}, shots=1024)
            result = task.result()
            probs = np.array(result.values).reshape(-1)
...
def page_curve_demo():
    ...
    for phi_val in timesteps:
        ...
        rm_circ, mapping = remap_circuit_to_contiguous_qubits(circ)
        task = device.run(rm_circ, inputs={"phi": phi_val}, shots=1024)
        result = task.result()
...
##    marg = marginal_probs(total_qubits=4, keep=keep)
