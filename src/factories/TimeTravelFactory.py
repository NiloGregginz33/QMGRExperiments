...
def run_and_extract_counts_qi(qc, backend, shots=1024):
    proxy = backend
    """Execute a quantum circuit on the QI backend and extract counts."""
    try:
        # Execute the circuit on the selected QI backend
        job = backend.run(qc, backend=proxy, shots=shots)
        result = job.result()
        counts = result.get_counts()
        return counts
    except Exception as e:
