...
try:
    ...
    best_backend = service.backend("ibm_kyiv")
...
def holographic_circuit():
    """
    Creates a quantum circuit that mimics lower-dimensional holographic encoding
    for a higher-dimensional bulk system.
    """
    # Define a 3-qubit circuit (1 qubit for boundary, 2 for bulk)
    qc = QuantumCircuit(3)

    # Bulk initialization: Create entanglement in the bulk
    qc.h(1)  # Apply Hadamard to qubit 1 (bulk qubit)
    qc.cx(1, 2)  # Entangle qubit 1 with qubit 2

    # Boundary encoding: Entangle bulk qubit with boundary
    qc.cx(1, 0)  # Entangle bulk qubit 1 with boundary qubit 0

    # Bulk interaction: Add a phase kick in the bulk
    qc.cp(0.5, 2, 1)  # Controlled phase gate between bulk qubits

    # Boundary measurement or projection
    qc.measure_all()  # Optional: Measure all qubits to analyze encoding
...
if __name__ == "__main__":
    ...
    qc = holographic_circuit()
    modify_and_run_quantum_experiment_multi_analysis(qc)
