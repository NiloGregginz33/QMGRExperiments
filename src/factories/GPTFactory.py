...
from qiskit.quantum_info import Statevector, partial_trace, entropy, DensityMatrix, state_fidelity, random_unitary
...
def extract_counts(result, use_sampler):
    ...
    if use_sampler:
        quasi_dists = result.quasi_dists[0]  # Extract first result
        total_shots = sum(quasi_dists.values())  # Normalize total counts
        counts = {key: int(round(value * total_shots)) for key, value in quasi_dists.items()}
    
    else:
        # Find the correct attribute inside DataBin
...
if __name__ == "__main__":
    backendd = initialize_backend(use_simulator=False)
    qc_test = main_qc_entangle()
    qc_test = create_entangled_circuit(qc_test)
    run_quantum_erasure_experiment()
