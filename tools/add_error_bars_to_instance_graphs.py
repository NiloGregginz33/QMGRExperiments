#!/usr/bin/env python3
"""
Add Error Bars to Instance Directory Graphs
==========================================

This script adds error bars to graphs in the main instance directory using
the error analysis data that's already available in the same directory.

Features:
- Reads error analysis results from error_analysis_results.json
- Creates enhanced versions of existing graphs with error bars
- Adds confidence intervals and statistical annotations
- Replaces existing graphs with enhanced versions
...
def create_enhanced_mi_plot(instance_dir: str, error_data: Dict):
    """Create enhanced MI plot with error bars."""
...
def create_enhanced_entropy_plot(instance_dir: str, error_data: Dict):
    """Create enhanced entropy plot with error bars."""
...
def create_mi_distance_correlation_with_errors(instance_dir: str, error_data: Dict):
    """Create enhanced MI vs Distance correlation plot with error bars."""
    ...
    # Check if we have error columns
    if 'mi_error' in df.columns and 'distance_error' in df.columns:
        # Create scatter plot with error bars
        plt.errorbar(df['distance'], df['mi'], 
                    xerr=df['distance_error'], yerr=df['mi_error'],
                    fmt='o', capsize=5, capthick=2, markersize=8, alpha=0.7,
                    label='Data points with errors', color='#F18F01')
    else:
    ...
    # Fit line with errors if enough data points
    if len(df) > 1:
        from scipy import stats
        
        # Use appropriate column names
        x_col = 'geometric_distance' if 'geometric_distance' in df.columns else 'distance'
        y_col = 'mutual_information' if 'mutual_information' in df.columns else 'mi'
...
def main():
    """Main function to add error bars to instance directory graphs."""
    if len(sys.argv) != 2:
        print("Usage: python add_error_bars_to_instance_graphs.py <instance_directory>")
        print("Example: python add_error_bars_to_instance_graphs.py experiment_logs/custom_curvature_experiment/instance_20250726_153536")
        return
    ...
    print(f"Adding error bars to graphs in: {instance_dir}")
    ...
    create_enhanced_mi_plot(instance_dir, error_data)
    create_enhanced_entropy_plot(instance_dir, error_data)
    create_mi_distance_correlation_with_errors(instance_dir, error_data)
    
    print("\n✅ Successfully added error bars to all graphs!")
    print("Enhanced plots saved in the instance directory with '_with_errors' suffix")
