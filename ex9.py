# Testing for color charge to understand fully why the green charge is so weird please run multiple lines with diff angles (***)
...
def create_color_charge_circuit(color_charge):
    ...
    if color_charge == "red":
        qc.rx((np.pi) / 4, 0)  # Rotate to simulate "red" charge (***)
    elif color_charge == "green":
        qc.ry((np.pi)/ 3, 0)  # Rotate to simulate "green" charge (***)
    elif color_charge == "blue":
        qc.rz((np.pi) / 4, 0)  # Rotate to simulate "blue" charge (***)
