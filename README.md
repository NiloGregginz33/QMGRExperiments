...
**All work in later internal/3rd party experimental files (eg. wedge comparison, multiverse holography comparisons) post date the first public disclosure and are considered non enabling. Matrix Solutions LLC is aware of the pre existing prior art on static wedge reconstruction.  The novel element first disclosed—as established by legal counsel—can be equivalently described as non-measurement information steering (Modifies properties of quantum bits without measurement or decoherence). As it is understood that quantum bit also appears in the dictionary of CV systems, and that information steering requires modifiying an unmeasured bit although measurement is allowed for readout or reconstruction or even figuring out if information steering worked at all (not an exhaustive list). This definition fully encompasses and supports the explicit embodiments and terminology used in the later provisional filing following the February patent search.
