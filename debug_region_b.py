#!/usr/bin/env python3
"""
Debug script for Region B decoding in EWR experiment
"""
...
def debug_region_b():
    """Debug Region B decoding specifically"""
    
    print("=== REGION B DECODING DEBUG ===")
    ...
    # Focus on Region B
    region_b_qubits = regions['B']
    print(f"\nRegion B qubits: {region_b_qubits}")
    print(f"Bulk point location: {bulk_point_location}")
    print(f"Is bulk point in Region B? {bulk_point_location in region_b_qubits}")
    
    # Create decoding circuit for Region B
    print(f"\nCreating decoding circuit for Region B...")
    decoding_circuits = create_decoding_circuit(region_b_qubits)
    ...
    # Check if the decoded state has the expected structure
    # For a successful decoding, we should see some non-zero amplitudes
    # in the computational basis states that correspond to the logical |1⟩ state
    ...
    target_qubit = region_b_qubits[0]
    print(f"Target qubit: {target_qubit}")
    
    # Analyze the decoded state for the target qubit
    # We want to see if the target qubit is in the |1⟩ state
    print(f"\nAnalyzing target qubit {target_qubit} state...")
    
    # Get the reduced density matrix for the target qubit
    target_state = decoded_state.partial_trace([i for i in range(12) if i != target_qubit])
    print(f"Target qubit reduced density matrix:")
    print(target_state.data)
    
    # Calculate the probability of measuring |1⟩ on the target qubit
    prob_1 = target_state.data[1, 1].real
    ...
    # The logical |1⟩ state should have significant amplitude
    logical_1_prob = 0.0
    for i, amp in enumerate(decoded_state.data):
        if abs(amp) > 1e-6:  # Non-zero amplitude
            ...
            # For a surface code, logical |1⟩ typically has odd parity
            binary = format(i, '012b')  # 12 qubits
