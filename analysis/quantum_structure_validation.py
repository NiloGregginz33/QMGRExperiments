...
Usage: python quantum_structure_validation.py <target_file>
...
def load_recent_data(data_path):
    """Load the analysis data from the specified path."""
...
def test_1_classical_geometry_benchmark(mi_matrix, num_qubits):
    ...
    random_mi = random_adj * np.random.uniform(0.01, 0.02, (n, n))
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                dist = abs(i - j)
                thermal_mi[i, j] = 0.02 * np.exp(-dist / 2.0)
    ...
    for name, matrix in geometries.items():
        dissimilarity = 1 - matrix / np.max(matrix)
...
def test_2_entropy_vs_classical_noise(mi_matrix, noise_levels=np.linspace(0, 0.5, 10)):
    ...
    for noise_level in noise_levels:
        ...
        dissimilarity = 1 - noisy_mi / np.max(noisy_mi)
        mds = MDS(n_components=2, random_state=42)
        coords = mds.fit_transform(dissimilarity)
        stress = mds.stress_
...
def test_4_entropy_curvature_link(entropies, bipartitions, mi_matrix):
    """
    ...
    for i in range(n):
        ...
        neighbors = []
        for j in range(n):
            if i != j and mi_matrix[i, j] > 0.01:  # Threshold for "connection"
                neighbors.append(j)
    ...
    # Group entropies by subsystem size
    size_entropy_map = {}
    for i, bipartition in enumerate(bipartitions):
        size = len(bipartition)
        if size not in size_entropy_map:
            size_entropy_map[size] = []
        size_entropy_map[size].append(entropies[i])
    ...
    # 2. Curvature vs subsystem entropy (for single qubits)
    if 1 in size_entropy_map:
        single_qubit_entropies = size_entropy_map[1]
        if len(single_qubit_entropies) >= n:
            corr, p_val = pearsonr(curvature, single_qubit_entropies[:n])
            correlations['curvature_vs_single_entropy'] = {'correlation': corr, 'p_value': p_val}
    
    # 3. Curvature vs MI matrix strength
    mi_strength = np.mean(mi_matrix)
    corr, p_val = pearsonr(curvature, [mi_strength] * n)  # Compare to global average
    correlations['curvature_vs_global_mi'] = {'correlation': corr, 'p_value': p_val}
    
    # 4. Entropy vs MI correlation by subsystem size
    for size, entropy_list in size_entropy_map.items():
        if len(entropy_list) > 1:
            # Compute average MI for this subsystem size
            size_mi_values = []
            for bipartition in bipartitions:
                if len(bipartition) == size:
                    # Compute average MI within this bipartition
                    if len(bipartition) >= 2:
                        subset_mi = mi_matrix[np.ix_(bipartition, bipartition)]
                        size_mi_values.append(np.mean(subset_mi))
            
            if len(size_mi_values) == len(entropy_list):
                corr, p_val = pearsonr(entropy_list, size_mi_values)
                correlations[f'entropy_vs_mi_size_{size}'] = {'correlation': corr, 'p_value': p_val}
...
def test_5_causal_violation_tracker(mi_matrix, circuit_depth=740):
    """
    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            if distance > max_causal_distance and mi_matrix[i, j] > 0.01:
    ...
    if causal_violations:
        ...
        violation_stats = {
            ...
            'quantum_signature': len(causal_violations) > 0 and np.mean(violation_strengths) > 0.01
        }
    else:
        violation_stats = {
            'num_violations': 0,
...
def create_validation_plots(test_results, output_dir):
    ...
    fig.suptitle('Quantum Structure Validation Tests', fontsize=16, fontweight='bold')
    ...
    regularities = [fit_metrics[name]['regularity'] for name in names]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'quantum_validation_tests.png'), dpi=300, bbox_inches='tight')
    plt.close()

def generate_validation_summary(test_results, output_dir):
    """Generate comprehensive validation summary."""
    
    summary = f"""
# Quantum Structure Validation Summary
...
in this system, distinguishing it from classical statistical patterns.

Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
"""
    
    with open(os.path.join(output_dir, 'quantum_validation_summary.txt'), 'w', encoding='utf-8') as f:
        f.write(summary)
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python quantum_structure_validation.py <target_file>")
        print("Example: python quantum_structure_validation.py experiment_logs/page_curve_comprehensive_analysis/instance_20250730_123456/comprehensive_analysis_results.json")
        sys.exit(1)
    ...
    # Extract data
    mi_matrix_str = data['analysis_results']['mi_matrix']
    # Parse the matrix string properly
    import re
    # Remove newlines and extra spaces
    mi_matrix_str = re.sub(r'\s+', ' ', mi_matrix_str.strip())
    # Convert to proper numpy array format
    mi_matrix_str = mi_matrix_str.replace('[', '[').replace(']', ']')
    # Split into rows and parse
    rows = mi_matrix_str.strip('[]').split('] [')
    mi_matrix = []
    for row in rows:
        row = row.strip('[]')
        row_values = [float(x) for x in row.split()]
        mi_matrix.append(row_values)
    mi_matrix = np.array(mi_matrix)
    
    # Load original page curve data for entropies and bipartitions
    page_curve_data = data['page_curve_data']
    entropies = page_curve_data['page_curve_analysis']['entropies']
    bipartitions = page_curve_data['page_curve_analysis']['bipartitions']
    num_qubits = page_curve_data['num_qubits']
    circuit_depth = page_curve_data['circuit_depth']
    ...
    test_results['test_4'] = test_4_entropy_curvature_link(entropies, bipartitions, mi_matrix)
    test_results['test_5'] = test_5_causal_violation_tracker(mi_matrix, circuit_depth)
    ...
    summary = generate_validation_summary(test_results, output_dir)
    ...
    with open(os.path.join(output_dir, 'quantum_validation_results.json'), 'w') as f:
